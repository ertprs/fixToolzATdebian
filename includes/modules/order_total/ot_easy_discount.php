      ...
      if ($easy_discount->count() > 0) {
        ...
        for ($i=0;$i < $n; $i++) {
           $this->output[] = array('title' => $easy_discounts[$i]['description'].': ',
                                   ...
                                   'value' => round($easy_discounts[$i]['amount'],2));
           $od_amount = $od_amount + $easy_discounts[$i]['amount'];
        }
        $od_amount = $easy_discount->total();
        //echo $easy_discount->total();
        $this->deduction = $od_amount;
	  $order->info['total'] = $cart->show_total() - round($od_amount,2);
        if ($this->sort_order < $ot_subtotal->sort_order) $order->info['subtotal'] = $cart->show_total() - round($od_amount,2);
      }
    }
