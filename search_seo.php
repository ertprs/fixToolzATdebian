...
var phpHelpUrl="<?php echo tep_href_link(FILENAME_ADVANCED_SEARCH_RESULT,'keywords=','NONSSL',false,false);?>";
  ...
  for(i=keyword.length-2; i>=0; i--)
  {
    ...
    {            
      ...
      for(j=0;j<cacheResults.length;j++)
      {
        ...
        if((cacheResults[j].toLowerCase()).indexOf(keyword.toLowerCase()) >= 0)   //== dibs            
  ...
  else
  {
    ...
    {  
      ...
      div += "<tr id='tr" + i + 
             ...
             "' href='" + phpHelpUrl + crtFunctionLink;
...
//               "</a></td></tr>";
        div += "'>" +
               (crtFunction.replace ( httpRequestKeyword.toUpperCase(), "<b>" + httpRequestKeyword.toUpperCase() + "</b>")) +
               "</a></td></tr>";
...
//  updateKeywordValue(document.getElementById("keyword"));
// EOF Search Suggest V2 Marcelon Brazil
...
<?php
  ...
  $info_box_contents[] = array('form' => tep_draw_form('quick_find', tep_href_link(FILENAME_ADVANCED_SEARCH_RESULT, '', 'NONSSL', false), 'get'),
                               'align' => 'center',
                               'text' => '<div id="content" onclick="hideSuggestions();"><div id="ie_fix">'.tep_draw_input_field('keywords', '', 'size="10" id="keyword" maxlength="60" onkeyup = "handleKeyUp(event)" style="width: ' . (BOX_WIDTH-30) . 'px"') .'&nbsp;'. tep_image_submit('button_quick_find.gif', BOX_HEADING_SEARCH).'</div>'.'<div id="scroll"><div id="suggest"></div></div></div>'. '&nbsp;' . tep_hide_session_id() . '<br>' . BOX_SEARCH_TEXT . '<br><a href="' . tep_href_link(FILENAME_ADVANCED_SEARCH) . '"><b>' . BOX_SEARCH_ADVANCED_SEARCH . '</b></a>'                               
                               );
