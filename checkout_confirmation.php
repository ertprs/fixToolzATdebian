...
*/
  ...
  require_once("includes/classes/LiveUpdate.class.php");
...
// load the selected shipping module
  ...
  require_once(DIR_WS_CLASSES . 'coupons.php');
    ...
    $osC_Coupon = new osC_Coupon($_POST['coupon_code']);
 
    if (!$osC_Coupon->is_valid_coupon) {
    	tep_redirect(tep_href_link(FILENAME_CHECKOUT_PAYMENT, 'error_message=' . urlencode(ERROR_INVALID_COUPON), 'SSL'));
      //echo "test";
      //echo ERROR_INVALID_COUPON;
      //echo tep_href_link(FILENAME_CHECKOUT_PAYMENT, 'error_message=' . urlencode(ERROR_INVALID_COUPON), 'SSL');
      //header( 'Location: http://www.google.com') ;
      //exit();
    }
    if ($osC_Coupon->is_valid_coupon == 'used') {
    	tep_redirect(tep_href_link(FILENAME_CHECKOUT_PAYMENT, 'error_message=' . urlencode(ERROR_USED_COUPON), 'SSL'));
    }

    if ($osC_Coupon->is_valid_coupon == 'true') {
...
?>
      <tr>
        ...
        <td><table border="0" width="100%" cellspacing="0" cellpadding="0">
          <tr>
            <td align="right" class="main" height="40">
<?php
  if (is_array($payment_modules->modules)) {
