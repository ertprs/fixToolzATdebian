...
// NOTE: Includes a workaround for dates before 01/01/1970 that fail on windows servers
  ...
  function tep_info_image($image, $alt, $width = '', $height = '', $subDir = '') {
    if (tep_not_null($image) && (file_exists(DIR_FS_CATALOG_IMAGES . $subDir . $image)) ) {
      $image = tep_image(DIR_WS_CATALOG_IMAGES . $subDir . $image, $alt, $width, $height);
    } else {
...
function tep_is_fake_category($id){
	...
	$query = tep_db_query("select c.categories_id, cd.categories_name from " . TABLE_CATEGORIES . " c left join categories_description cd on cd.categories_id = c.categories_id where c.parent_id = 0 ORDER BY c.categories_id");
    $i=0;
    while($categories = tep_db_fetch_array($query)){
    	if($i!=0 && $categories['categories_id'] == $id && $categories['categories_name']!="Solduri"){ 
    		$return = true;
    	}
    	$i++;
    }
...
function tep_set_coupon_status($coupon_id, $status) {
    if ($status == '1') {
      return tep_db_query("update " . TABLE_COUPONS . " set coupon_status = '1' where coupon_id = '" . $coupon_id . "'");
    } elseif ($status == '0') {
      return tep_db_query("update " . TABLE_COUPONS . " set coupon_status = '0' where coupon_id = '" . $coupon_id . "'");
    } else {
      return -1;
    }
  }
  ...
  function tep_get_coupon_type($coupon_type_id) {
    if ($coupon_type_id == '1') {
      return ENTRY_FIXED;
    } else {
  ...
  function tep_get_coupon_use($coupon_use_id) {
    if ($coupon_use_id == '1') {
      return ENTRY_MULTI;
    } else {
      return ENTRY_ONCE;
