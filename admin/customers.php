  ...
  if (tep_not_null($action)) {
    switch ($action) {
      case 'update':
    	...
    	$customers_cnp = tep_db_prepare_input($_POST['cnp']);
    	$customers_ci = tep_db_prepare_input($_POST['ci']);
...
//        $entry_city = tep_db_prepare_input($_POST['entry_city']);
	    ...
	    $entry_reg_com = tep_db_prepare_input($_POST['reg_com']);
	    $entry_cui = tep_db_prepare_input($_POST['cui']);
	    $entry_iban = tep_db_prepare_input($_POST['iban']);
        ...
        if (ACCOUNT_DOB == 'true') {
      	  if (checkdate(substr(tep_date_raw($customers_dob), 4, 2), substr(tep_date_raw($customers_dob), 6, 2), substr(tep_date_raw($customers_dob), 0, 4)) == false) {
            $error = true;
            $entry_date_of_birth_error = true;
          } else {
            $entry_date_of_birth_error = false;
          }
          //echo tep_date_raw($customers_dob);
        }
...
//        }
      ...
      if ($error == false) {
        ...
        $sql_data_array = array('customers_firstname' => $customers_firstname,
                                ...
                                'customers_cnp' => $customers_cnp,
                                'customers_ci' => $customers_ci,
...
//                                'entry_city' => $entry_city,
        ...
        if (ACCOUNT_COMPANY == 'true') $sql_data_array['entry_reg_com'] = $entry_reg_com;
        if (ACCOUNT_COMPANY == 'true') $sql_data_array['entry_cui'] = $entry_cui;
        if (ACCOUNT_COMPANY == 'true') $sql_data_array['entry_iban'] = $entry_iban;
      ...
      case 'deleteconfirm':
        ...
        break;
      default:
        $customers_query = tep_db_query("select c.customers_id, c.customers_gender, c.customers_firstname, c.customers_lastname, c.customers_dob, c.customers_cnp, c.customers_ci, c.customers_email_address, a.entry_company, a.entry_reg_com, a.entry_cui, a.entry_iban, a.entry_street_address, a.entry_suburb, a.entry_state, a.entry_zone_id, a.entry_country_id, c.customers_telephone, c.customers_fax, c.customers_newsletter, c.customers_default_address_id from " . TABLE_CUSTOMERS . " c left join " . TABLE_ADDRESS_BOOK . " a on c.customers_default_address_id = a.address_book_id where a.customers_id = c.customers_id and c.customers_id = '" . (int)$_GET['cID'] . "'");
...
?></td>
          ...
          <tr>
            <td class="main">CNP: </td>
            <td class="main">
<?php
  if ($error == true) {
    if ($entry_lastname_error == true) {
      echo tep_draw_input_field('customers_cnp', $cInfo->customers_cnp, 'maxlength="32"') . '&nbsp;' . ENTRY_LAST_NAME_ERROR;
    } else {
      echo $cInfo->customers_cnp . tep_draw_hidden_field('customers_cnp');
    }
  } else {
    echo tep_draw_input_field('customers_cnp', $cInfo->customers_cnp, 'maxlength="32"', true);
  }
?></td>
          </tr>
          <tr>
            <td class="main">Serie si Numar CI:</td>
            <td class="main">
<?php
  if ($error == true) {
    if ($entry_lastname_error == true) {
      echo tep_draw_input_field('customers_ci', $cInfo->customers_ci, 'maxlength="32"') . '&nbsp;' . ENTRY_LAST_NAME_ERROR;
    } else {
      echo $cInfo->customers_ci . tep_draw_hidden_field('customers_ci');
    }
  } else {
    echo tep_draw_input_field('customers_ci', $cInfo->customers_ci, 'maxlength="32"', true);
  }
?></td>
          </tr>
<?php
...
?></td>
          ...
          <tr>
            <td class="main">Nr. Registrul Comertului: </td>
            <td class="main">
<?php
    if ($error == true) {
      if ($entry_company_error == true) {
        echo tep_draw_input_field('entry_reg_com', $cInfo->entry_reg_com, 'maxlength="32"') . '&nbsp;' . ENTRY_COMPANY_ERROR;
      } else {
        echo $cInfo->entry_reg_com . tep_draw_hidden_field('entry_reg_com');
      }
    } else {
      echo tep_draw_input_field('entry_reg_com', $cInfo->entry_reg_com, 'maxlength="32"');
    }
?></td>
          </tr>
          <tr>
            <td class="main">CUI: </td>
            <td class="main">
<?php
    if ($error == true) {
      if ($entry_company_error == true) {
        echo tep_draw_input_field('entry_cui', $cInfo->entry_cui, 'maxlength="32"') . '&nbsp;' . ENTRY_COMPANY_ERROR;
      } else {
        echo $cInfo->entry_company . tep_draw_hidden_field('entry_cui');
      }
    } else {
      echo tep_draw_input_field('entry_cui', $cInfo->entry_cui, 'maxlength="32"');
    }
?></td>
          </tr>
          <tr>
...
<?php
    if ($error == true) {
      if ($entry_company_error == true) {
        echo tep_draw_input_field('entry_iban', $cInfo->entry_iban, 'maxlength="32"') . '&nbsp;' . ENTRY_COMPANY_ERROR;
      } else {
        echo $cInfo->entry_iban . tep_draw_hidden_field('entry_iban');
      }
    } else {
      echo tep_draw_input_field('entry_iban', $cInfo->entry_iban, 'maxlength="32"');
    }
?></td>
          </tr>
        </table></td>
...
<?php
  if ($error == true) {
    if ($entry_street_address_error == true) {
      echo tep_draw_textarea_field('entry_street_address', 'soft','30','5',$cInfo->entry_street_address, 'maxlength="64"') . '&nbsp;' . ENTRY_STREET_ADDRESS_ERROR;
    } else {
      echo $cInfo->entry_street_address . tep_draw_hidden_field('entry_street_address');
    }
  } else {
    echo tep_draw_textarea_field('entry_street_address', 'soft','30','5', $cInfo->entry_street_address, 'maxlength="64"', true);
  }
?></td>
          </tr>
<?php
...
<?php
    ...
    while ($customers = tep_db_fetch_array($customers_query)) {
      $info_query = tep_db_query("select customers_info_date_account_created as date_account_created, customers_info_date_account_last_modified as date_account_last_modified, customers_info_date_of_last_logon as date_last_logon, customers_info_number_of_logons as number_of_logons, customers_info_ip as created_ip  from " . TABLE_CUSTOMERS_INFO . " where customers_info_id = '" . $customers['customers_id'] . "'");
...
<?php
  ...
  switch ($action) {
	...
	$contents[] = array('text' => '<br> Cont creat de pe adresa de IP: ' . $cInfo->created_ip);
