  ...
  if (tep_not_null($action)) {
    switch ($action) {
      ...
      case 'save':
        ...
        //echo (DIR_FS_CATALOG_MANUFACTURERS);
        $manufacturers_image = new upload('manufacturers_image');
        $manufacturers_image->set_destination(DIR_FS_CATALOG_MANUFACTURERS);
        
        if ($manufacturers_image->parse() && $manufacturers_image->save() && $manufacturers_image->resize(150, 50) ) {
      ...
      case 'deleteconfirm':
        ...
        if (isset($_POST['delete_image']) && ($_POST['delete_image'] == 'on')) {
          ...
          $image_location = DIR_FS_DOCUMENT_ROOT . DIR_FS_CATALOG_MANUFACTURERS . $manufacturer['manufacturers_image'];
...
//        if (tep_not_null($mInfo->last_modified)) $contents[] = array('text' => TEXT_LAST_MODIFIED . ' ' . tep_date_short($mInfo->last_modified));
        ...
        $contents[] = array('text' => '<br>' . 'ID Producator: '. $mInfo->manufacturers_id);
        $contents[] = array('text' => '<br>' . tep_info_image($mInfo->manufacturers_image, $mInfo->manufacturers_name, '', '', 'manufacturers/'));
        $contents[] = array('text' => '<br>Produse: ' . $mInfo->products_count);                           
      }
      break;
  }
